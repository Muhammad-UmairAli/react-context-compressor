...
- **Never run `tools/init-project.sh` from inside Claude Code.** It requires a real terminal — it uses `date +%s.%N` timing and interactive prompts. If Phase 0 rows are missing, tell the user to run the script from their terminal.
